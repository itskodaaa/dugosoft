...
const PLAN_LIMITS = {
  free:     { ai_requests: 10, pdf_count: 5, ocr_count: 3, translations: 2 },
  pro:      { ai_requests: 200, pdf_count: 100, ocr_count: 50, translations: 999 },
  business: { ai_requests: 999, pdf_count: 999, ocr_count: 999, translations: 999 },
};

const METERS = [
  { key: "ai_requests", label: "AI Requests",     icon: Zap,       color: "from-accent to-blue-500",          bg: "bg-accent/10",    text: "text-accent" },
  { key: "pdf_count",   label: "PDF Operations",  icon: FileText,  color: "from-orange-500 to-amber-400",     bg: "bg-orange-100",   text: "text-orange-600" },
  { key: "ocr_count",   label: "OCR Scans",       icon: ScanText,  color: "from-purple-500 to-violet-400",   bg: "bg-purple-100",   text: "text-purple-600" },
    ...
    <div className="bg-card ink-border rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold text-foreground">Today's Usage</p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${
            ...
            {plan === "business" ? "🏆 " : plan === "pro" ? "⚡ " : ""}{plan.charAt(0).toUpperCase() + plan.slice(1)}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {METERS.map((m, i) => {
          ...
          const limit = limits[m.key] || 0;
          const pct = isUnlimited || limit >= 999 ? 5 : (used / limit) * 100;
          const nearLimit = pct >= 80 && !isUnlimited && limit < 999;
        ...
        <Link to="/dashboard/pricing">
          <div className="mt-4 flex items-center gap-2 p-3 bg-gradient-to-r from-accent/10 to-green-500/10 rounded-xl cursor-pointer hover:from-accent/15 transition-colors border border-accent/20">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground">Pro: 200 AI · 100 PDF · unlimited translation</p>
            </div>
          </div>
        </Link>
      )}
